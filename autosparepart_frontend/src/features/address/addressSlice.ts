import { Address, AddAddressRequest, UpdateAddressRequest } from "@/types";
...
export const addAddress = createAsyncThunk(
  ...
  async ({ request }: { request: AddAddressRequest }, { getState }) => {
    const state: any = getState();
    const token = state.auth.token;
...
export const updateAddress = createAsyncThunk(
  ...
  async (
    { addressId, address }: { addressId: number; address: AddAddressRequest },
    { getState }
  ) => {
    const state: any = getState();
    const token = state.auth.token;
      ...
      .addCase(
        ...
        (state, action: PayloadAction<Address>) => {
          ...
          const index = state.items.findIndex(
            (item) => item.addressId === action.payload.addressId
          );
          if (index && index !== -1) {
